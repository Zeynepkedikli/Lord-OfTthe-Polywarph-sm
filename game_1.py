...
class GameGUI:
    def __init__(self, master):
        ...
        self.players = [Player(player_id) for player_id in
                        range(1, self.player_count + 1)]  # Oyuncuları saklamak için bir liste oluşturuluyor
    ...
    def initialize_guardians(self):
        ...
        corners = [(0, 0), (0, self.world_size - 1), (self.world_size - 1, 0),
            ...
            if current_color == player_color:
                ...
                for dr in [-1, 0, 1]:
                    for dc in [-1, 0, 1]:
                        ...
                        if adjacent_cell_key in self.cells and (dr != 0 or dc != 0) and (
                                self.cells[adjacent_cell_key]["bg"] == "white"):
            ...
            for warrior in player.warriors:
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ...
                        if 0 <= new_x < self.world_size and 0 <= new_y < self.world_size and (
                                self.cells[new_x, new_y]["bg"] == "white"):
                            self.cells[new_x, new_y].config(bg=self.get_warrior_color(player.player_id))
                self.cells[warrior.x, warrior.y].config(text=warrior.name[0] + str(warrior.player_id),
                                                        bg=self.get_warrior_color(player.player_id))

                # Oyuncu bilgilerini güncelle
        for i, player in enumerate(self.players):
            self.player_info_labels[i].config(
                text=f'Player {i + 1}: Health: {player.health}, Resources: {player.resources}')
    ...
    def create_player_info_panel(self):
        ...
        for i in range(self.player_count):
            label = tk.Label(self.info_panel,
                             text=f'Player {i + 1}: Health: ' + str(self.players[i].health) + ', Resources: ' + str(
                                 self.players[i].resources))  # Örnek değerler
    ...
    def create_board(self):
        ...
        for row in range(self.world_size):
            for col in range(self.world_size):
                cell = tk.Button(board_frame, text='', width=4, height=2,
    ...
    def cell_clicked(self, row, col):
        ...
        select_button = tk.Button(selection_window, text="Select",
                                  command=lambda: on_warrior_selected(selection_window))
        select_button.pack()
